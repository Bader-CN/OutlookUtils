                ...
                if attachment.FileName == raw_case_report_name:
                    ...
                    if month_offset == 0 and (len(backlog) + len(backlog_history)) >= 0:
                        ...
                        if len(ssdata_os) != 0:
                            ...
                            summary_datas.append(["DTR Support only", dtr_os_avg])
                        else:
                            summary_datas.append(["DTR Support only", "-"])
                    else:
                    ...
                    # Escalated
                    if len(open_cases_m) != 0:
                        summary_datas.append(["Escalated", str(len(open_cases_m[open_cases_m.Escalated != 0]))])
                    # 删除文件
                    try:
            ...
            for attachment in surv_list_obj:
                if attachment.FileName == raw_surv_report_name:
                    ...
                    if len(survey_m) > 0 and clos_cases_by_month is not None:
                        summary_datas.append(["Survey Response Rate", str(round(len(survey_m) / len(clos_cases_by_month) * 100, 2)) + "%"])
                    else:
                        summary_datas.append(["Survey Response Rate", "-"])
                    # 删除文件
                    try:
